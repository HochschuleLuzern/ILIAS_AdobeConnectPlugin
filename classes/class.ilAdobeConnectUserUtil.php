...
class ilAdobeConnectUserUtil
{
	...
	private function readAdobeConnectUserData()
	{
		...
		$result = $ilDB->queryF("SELECT email,login,passwd,firstname,lastname FROM usr_data WHERE usr_id= %s", array(
			'integer'
		), array(
			$this->getId()
		));
		...
		$login_res = $ilDB->queryf('
			SELECT xavc_login FROM rep_robj_xavc_users WHERE user_id = %s', array(
			'integer'
		), array(
			$this->getId()
		));
	...
	private function ensureLocalAccountExistance()
	{
		...
		if (! $this->xavc_login && $expected_login_name) // || $this->xavc_login != $expected_login_name)
		{
			...
			$ilDB->replace('rep_robj_xavc_users', array(
				'user_id' => array(
					'integer',
					$this->getId()
				)
